...
class ADPCog(commands.Cog):
    ...
    async def send_all_narffl_adp_posts(self, interaction: discord.Interaction,
        ...
        await interaction.response.defer()
        await self._post_narffl_position_adp(forum,"D/ST", "Team Defense", DEF_COLOR)
        await self._post_narffl_position_adp(forum, "K", "Kicker", K_COLOR)
        await self._post_narffl_position_adp(forum, "TE", "Tight End", TE_COLOR)
        await self._post_narffl_position_adp(forum, "WR", "Wide Receiver", WR_COLOR)
        await self._post_narffl_position_adp(forum, "RB", "Running Back", RB_COLOR)
        await self._post_narffl_position_adp(forum, "QB", "Quarterback", QB_COLOR)
        await self._post_narffl_position_adp(forum, adp.INCLUDE_ALL, "All Players", ALL_PLAYERS_COLOR)
